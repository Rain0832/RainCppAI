...
{
    SPDLOG_DEBUG_TAG("HTTP") << "CorsMiddleware::before - Processing request";

    if (request.method() == HttpRequest::Method::kOptions)
    {
        SPDLOG_INFO_TAG("HTTP") << "Processing CORS preflight request";
...
{
    SPDLOG_DEBUG_TAG("HTTP") << "CorsMiddleware::after - Processing response";
...
void CorsMiddleware::handlePreflightRequest(const HttpRequest& request, HttpResponse& response)
{
    ...
    {
        SPDLOG_WARN_TAG("HTTP") << "Origin not allowed: " << origin;
    ...
    SPDLOG_INFO_TAG("HTTP") << "Preflight request processed successfully";
}
...
void CorsMiddleware::addCorsHeaders(HttpResponse& response, const std::string& origin)
{
    try
    {
        ...
        SPDLOG_DEBUG_TAG("HTTP") << "CORS headers added successfully";
    }
    catch (const std::exception& e)
    {
        SPDLOG_ERROR_TAG("HTTP") << "Error adding CORS headers: " << e.what();
    }
}
