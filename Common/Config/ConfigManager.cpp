...
void ConfigManager::load(const std::string& path)
{
    ...
    if (!file.is_open())
    {
        std::cerr << "[ConfigManager] Cannot open config file: " << path << ", using defaults" << std::endl;
        config_ = json::object();
        return;
    }
    file >> config_;
...
{
    json node = config_;
    if (path.empty() || node.is_null()) return node;
    ...
    while (dot != std::string::npos)
    {
        ...
        if (!node.contains(key)) return json();
        node = node[key];
        start = dot + 1;
        dot = path.find('.', start);
    }
    std::string lastKey = path.substr(start);
    if (!node.contains(lastKey)) return json();
    return node[lastKey];
}
